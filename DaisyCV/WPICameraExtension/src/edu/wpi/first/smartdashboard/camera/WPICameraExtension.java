...
public class WPICameraExtension extends StaticWidget {
        ...
        public void run() {
            ...
            while (!destroyed) {
                ...
                try {
                    ...
                    drawnImage = null;
                    repaint();
                    System.out.println("Could not connect to camera! (From the "
                    		+ "BadConnectionException thrown by the WPICamera, which "
                    		+ "is a subclass of WPIFFmpegVideo.");
                    try {
                        Thread.sleep(2000);
                    } catch (InterruptedException ex) {
                    }
    ...
    public final StringProperty ipProperty = new StringProperty(this, "Camera IP Address", "axis-camera.local");
